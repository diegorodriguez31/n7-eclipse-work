...
public class SimplePDLToPetriNet {
	...
	public static void generatePetriNet(Process process, String modelPath) {
	    ...
	    PetrinetFactory myFactory = PetrinetFactory.eINSTANCE;
 		...
 		process.getProcessElements().stream().forEach(pe -> {
 			if (pe instanceof WorkDefinition) {
 				...
 				Arc aws = myFactory.createArc();
 	 			...
 	 			aws.setSource(
				(Node) petrinet.getPetriElements().stream().filter(p -> p instanceof Node).filter(
					n -> ((Node) n).getName().equals(
						ws.getPredecessor().getName() + (
							ws.getLinkType() == WorkSequenceType.START_TO_START
							|| ws.getLinkType() == WorkSequenceType.START_TO_FINISH 
							? "_started" 
							: "_finished"
						)
					)
 	 			...
 	 			aws.setTarget((Node) petrinet.getPetriElements().stream().filter(p -> p instanceof Node).filter(
 					n -> ((Node) n).getName().equals(
 							ws.getSuccessor().getName() + (
								ws.getLinkType() == WorkSequenceType.START_TO_START
								|| ws.getLinkType() == WorkSequenceType.FINISH_TO_START 
								? "_start" 
								: "_finish"
							)
 						)
 				...
 				for (Need ne : rs.getNeeds()) {
 					Arc anLoad = myFactory.createArc();
 	 				...
 	 				anLoad.setTarget((Node) petrinet.getPetriElements().stream().filter(p -> p instanceof Node).filter(
 						n -> ((Node) n).getName().equals(
							ne.getWorkdefinition().getName() + "_start"
						)
 	 	 			).toArray()[0]);
 		...
 		// On enregistre le modèle petrinet
	    try {
	...
	public static void main(String[] args) {
		if (args.length != 2)
			throw new IllegalArgumentException("Usage: java SimplePDLToPetriNet <simplepdl_model_path> <petrinet_model_path>");
